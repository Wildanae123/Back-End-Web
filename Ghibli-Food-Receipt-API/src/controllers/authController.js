...
const parseExpiryToMilliseconds = (expiresInString) => {
  ...
  switch (unit) {
    ...
    default: return value; // Assume it's already in ms if no unit or unknown unit
  }
};
    ...
    if (req.cookies && req.cookies.token) {
      try {
        ...
        if (userFromCookie) {
          ...
          const newToken = generateToken(userFromCookie.id, userFromCookie.role);
        ...
        console.log('Invalid or expired existing cookie, clearing and proceeding with login attempt.');
